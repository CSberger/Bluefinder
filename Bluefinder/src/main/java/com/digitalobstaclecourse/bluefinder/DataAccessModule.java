...
//		ModelStoreLocationData m = new ModelStoreLocationData(stored_loc, new Date());
	...
	public String getMostRecentLocationForDevice(String device_id) {
		SQLModelOpener opener = new SQLModelOpener(this.c);
		SQLiteDatabase db = opener.getReadableDatabase();		
		String query = "SELECT locationCoords FROM LocationTable where locationDate = (select max(locationDate) from LocationTable WHERE deviceTableKey = ?)";
